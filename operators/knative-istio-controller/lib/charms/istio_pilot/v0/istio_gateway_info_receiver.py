...
class GatewayConsumer(Object):
    ...
    def get_relation_data(self):
        ...
        remote_app = [app for app in gateway[0].data.keys() if isinstance(app, Application) and not app._is_our_app][0]
        
        data = gateway[0].data[remote_app]
        
        if not "gateway_name" in data:
            self.log.error("Missing gateway name in gateway relation data. Waiting for gateway creation in istio-pilot")
            raise Exception("Missing gateway name in gateway relation data. Waiting for gateway creation in istio-pilot")
        ...
        return {"gateway_name": data["gateway_name"], "gateway_namespace": data["gateway_namespace"]}
